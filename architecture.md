...
| `/my/cohorts/[cohortId]/report` | 멤버(user) | 내 리포트 **순화 뷰**(Step 1.3, ADR-27/30). `listResponses`(본인 pre·post, self-read)→각 wave `latestPerUser`→score→`participantMirror`→`MirrorView`. **사전·사후 모두 있으면 미러 2개 나란히 비교(B-3·ADR-57), 하나면 단독**. 측정·severity 0(순화 유지). 계약·DB 무변경 |
