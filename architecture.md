...
> **S-1·S-2 인증 왕복 최적화(검증 신뢰 실증, 2026-07-02)**: S-1 이중 getUser dedup(ADR-66 — proxy 검증 신원 헤더 전달·strip 신뢰 경계·요청당 Auth 왕복 2→1·role SELECT 유지·세션 갱신 유지, 위조/미인증 거부 로컬 실증). 대기: S-2 getClaims 교체(세션 갱신 제약 선결 실증 후 지휘부 확정 — role 클레임화 배제). **이중 getUser(proxy↔page)·getClaims 검증 교체는 보안 사안 — 별도 보고·승인 후.**
