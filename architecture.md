...
`.gitignore` 에 `/docs/` 로 앵커한다(앞 슬래시가 없으면 하위 디렉터리의 다른 `docs` 까지 함께 막힌다). 이 규율은 저장소 공개 여부와 무관하다 — private 으로 바꿔도 협업자가 늘면 접근이 함께 넓어지고 이력은 여전히 지우기 어렵다. **녹취록은 버전 관리가 필요한 물건이 아니다.**
