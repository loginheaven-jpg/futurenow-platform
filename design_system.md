...
### 9.7 공개·홈 부품 9 (v5 신설 · 4차 F-1 · 발주 §3)

`src/app/_screens/site/`. **공개 현관·로그인 홈 전용 계층**이고 콘솔·측정 부품과 섞지 않는다.

**아홉 부품에 공통으로 걸리는 한 줄** — **부품은 계산하지 않는다.** 현재 회차·잠금·완료·현재 경로가
...
- **참여자 앱 하단 탭바 — 근거 하나가 사라졌다(4차 F-1 · 질의)**. §9.6 은 *"두지 않는다"* 로
  확정돼 있고 그 결정은 그대로다. 다만 그 근거 중 *"시안에도 없다"* 가 **사실이 아니었다** —
  시안 v1.4 **B·C·F 가 4칸 탭바**(홈·내 여정·피드·자료실), G 가 4칸(홈·소식·자료실·계정)을 갖는다.
  3차 T-1 에서 A 화면의 5칸만 보고 단정했다. **사실 칸은 고쳤고 결정은 손대지 않았다.**
  → **지휘부 판단 필요**: 근거가 하나 줄어든 상태에서 "두지 않는다"를 유지할 것인가,
  아니면 F-2·F-3 화면 조립 때 다시 볼 것인가. 무응답 시 **유지**로 두고 임의로 만들지 않는다.
