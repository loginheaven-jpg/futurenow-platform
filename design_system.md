...
> 참조: architecture.md §10·§8·§5.2·§1. ADR-04·20·133·139.
...
1. **색은 3단으로만 흐른다**(ADR-139). 원천 hex → 역할 → 컴포넌트. 컴포넌트는 hex·원천 토큰 직접 참조 금지.
   <!-- 문장은 2026-06-27 확정 그대로이고 **번호만 2026-08-29 에 새로 받았다**(내용 변경 없음).
        원래 `(ADR-11)` 로 적혀 있었으나 **ADR-11 은 폐기된 `core` 스키마 격리 건**이고 색과 무관하다 —
        오귀속이었다. 경위는 ADR-139 본문에, 반대 방향 관계는 ADR-11 의 정정 주에 있다. -->
...
| 8 | **`SessionChipStrip`** | 회차 칩. 완료·진행·열림·잠금 4상태 | 완료 `--color-primary` 면 + 체크 · 진행 `--color-accent` 면 + `text-on-gold` · **열림 `--color-primary` 테두리·글자(면 없음)** · 잠금 점선 + `surface-sunken` + muted | **잠긴 회차를 감추지 않는다.** 상태를 **색만으로** 말하지 않는다(`aria-label`) — 네 단이 «면 → 면 → 테두리 → 점선» 으로 **형태로도** 내려간다(ADR-182 · 지휘부 가독성 승인 2026-09-03). 의미색 0(불변식 9) | `SessionChipStrip.tsx` |
| 9 | **`SiteGnb`** | 네이비 상단 바. **두 모습** — `public`(시안 P1 `.gnb`: 로고 · 메뉴 6 · 골드 로그인) / `member`(시안 B `.topbar`: **로고 + 햄버거뿐**) | `public` md↑ 메뉴 · md↓ 햄버거 · `member` 는 폭과 무관하게 햄버거(시트가 내비다) · `aria-current="page"` | 현재 경로 판정을 부품이 하지 않는다 — **`currentPath` prop**(라우터를 읽지 않는다). **로그인한 사람에게 로그인 버튼을 다시 보이지 않는다** · **v2(ADR-174): `member` 도 메뉴를 든다**(로그인해도 벨트가 안 사라진다) · 시트를 받으면 여는 문이 md↑ 에서도 선다(`has-sheet`) · **제목바 화면 위에 `belt-slot` 으로 얹힌다(lg↑)** | `SiteGnb.tsx` |
...
#### 12.6-b **U-5 — 제목바를 걷고 회기 띠로 간다**(지휘부 결재 2026-09-03 · ADR-189)

U-3 이 세운 것은 **띠 셋**이었다 — 벨트 52 · 제목바 58 · 탭 줄 52 = **162px**(실측).
지휘부 지적: *「2열이 없어도 3열에서 현재 위치가 표시되는데?」* — **탭이 이미 «지금 어디»를
말하므로 제목바는 같은 말을 두 번**이었다. 참여자에게 없는 겹이 인도자에게만 있을 이유가 없다.

| 제목바가 들던 것 | 어디로 갔는가 |
|---|---|
| **제목** | 회기 화면은 **탭**이, 회기 밖 화면(`/coach`·`/coach/cohorts`·`/coach/new`·`/admin`·`/admin/approvals`)은 **본문 첫 줄**(`.console-title`)이 든다. 이름은 여전히 `SCREEN_CHROME` 표에서만 온다 — 화면이 손으로 적지 않는다 |
| **뒤로** | **폐지**(결재 「뒤로가기는 불필요」). 벨트와 탭이 갈 곳을 든다 |
| **도구**(`HeaderActions`) | **시트**로. 「내 정보」는 계정 구획에 있다 |
| **햄버거** | **벨트 하나로.** 전에는 벨트와 제목바에 각각 있어 한 화면에 문이 둘이었다(U-4 §4 를 회원 껍데기에서만 고쳤고 콘솔은 남아 있었다) |

**띠 둘이 된다** — 벨트 + **회기 띠**. 회기 띠 왼쪽에 **회기 칩**이 서고 오른쪽에 탭이 흐른다.

- **칩은 부제가 아니라 문**이다 — 누르면 `/coach/cohorts`(회기 목록)로 간다.
  펼침 목록은 새 부품이 필요해 **불변식 20** 에 걸린다(결재 물음 1 답).
- **칩은 껍데기가 아니라 `/coach/cohort/[cohortId]/layout.tsx` 가 그린다.** 회기 이름은
  **서버 데이터**라 `/coach/layout` 이 알 수 없고(그 레이아웃은 `[cohortId]` 세그먼트 **위**에 있다),
  통로(`useSetChrome`)로 올리면 `useEffect` 라 **첫 그림에 이름이 없어** 칩이 늦게 끼어들며 탭을 민다.
- **선택 탭은 골드 면이 아니라 흰 밑줄**이다(결재 2·3) — 골드는 §1.5 에서 **참여자의 흔적**이라
  인도자 탭이 쓰면 한 색이 두 뜻을 갖는다. 색만으로 말하지 않는 규율(§10 v4)은 그대로다
  — 흰 글자 + 아래 띠 + `aria-current` 셋을 함께 쓴다.
- **탭 이름에서 「회기」를 뺐다** — 칩이 이미 그것을 말한다(「회기 대시보드」 → 「대시보드」).

**시트는 참여자와 같은 네 구획**이다 — 「내 자리」(콘솔 · 본부 · 퓨처나우) · 「인도자」(모든 차수 ·
차수 개설 · 가입 승인) · 「자료」(서가 · 소식) · 「계정」(내 정보 + 로그아웃).
「운영」 구획은 **「내 자리」에 합쳤고**(결재), 「내 홈」은 **뺐다** — 인도자의 홈이 곧 이 화면이다.

⚠ **최박사 결재 2026-09-01 과 부딪히지 않게 지은 자리**: 그 결재는 *「`subtitle` 은 표가 들지 않고
**본문이 든다**」* 이다. U-5 는 `subtitle` 을 **쓰지 않고**(`tests/chromeContext.test.tsx` 가 인도자
화면에서 그것을 잠근다) `/matrix`·`/values`·`/group` 의 **본문 첫 줄 회기 이름도 지우지 않았다.**
→ **회기 이름이 두 곳(칩·본문)에 보이는 화면 셋이 남는다. 어느 쪽을 지울지는 최박사 재가 사항**이다.

#### 12.6-c **U-6 — 이름은 정확히 한 곳에**(지휘부 결재 2026-09-03 · ADR-190)

지휘부 목표: *「모든 기능을 **빠짐없이** 제공하되 **중복없이**, **일관된 위치**에서」*

| 무엇을 말하는가 | 어디가 그것을 든다 | 언제 |
|---|---|---|
| **어느 회기인가** | **띠의 칩**(`ConsoleBand`) | 회기 안 여덟 화면 전부 |
| **그 회기 안 어디인가** | **탭**(`cohortTabs`) | 탭이 켜지는 넷 |
| **이 화면 이름** | **본문 첫 줄**(`ConsoleTitle`) | **탭이 안 켜지는 화면 전부** |

이름은 언제나 `SCREEN_CHROME` 표에서만 온다 — 화면이 손으로 적지 않는다(사본 0).
`tests/consoleNames.test.ts` 가 **라우트 전수**로 이 표를 잠근다.

**착수 전 실측이 규칙을 요구했다.**
- 이름의 자리가 **넷으로 흩어져** 있었고 그중 하나가 «아무 데도 없음» 이었다 —
  `/group`·`/member/[userId]`·`/report/[responseId]`·`/checkin/preview` **넷은 화면 이름이 없었다.**
  띠는 그 넷에서 최장일치로 엉뚱한 탭을 켜 둔 채여서 사용자가 자기 위치를 오해했다.
- 거꾸로 **회기 이름은 셋에서 겹쳤다**(`/matrix`·`/values`·`/member` — 칩과 본문).
- **표와 탭이 다른 이름을 달던 라우트가 셋**이었고, 그중 `/matrix` 는 **탭 이름이 화면의 실체와 달랐다**
  (`listCohortCheckins` 로 그리는 **갈무리 격자**인데 탭만 「진단 결과」).

**제목의 자리가 껍데기에서 화면 컨테이너 «안»으로 옮겨졌다.**
콘솔 본문 폭이 화면마다 다르고(**∞ · 480 · 480 · 560 · 1200** — 실측) 껍데기는 그중 하나를 고를 수밖에 없어
U-5 에서 **다섯 중 넷이 어긋났다**(1280px 기준 제목-본문 좌측 ΔL **−56 · +360 · +360 · +320 · 0**).
제목은 화면 컨테이너의 **형제**라 그 폭을 상속받지 못한다 — 자리를 옮기는 것 말고 처방이 없다.
폭 자체도 인라인 사본 다섯을 **`.console-body`**(`--mid` 560 · `--wide` 1200) 한 곳으로 걷었다.
`/group` 은 리포트 판형이라 자기 폭(900)을 그대로 쓴다.

**어휘 — 「차수」·「기수」가 「회기」로 갔다**(전면).
ADR-182 는 「어휘 전면 · 화면 문안 24건」을 선언했으나 그 잠금은 **네 파일만** 보고 있었다.
U-6 실측은 화면에 나가는 옛 낱말이 **111자리**였다 — 판정의 창이 판정하려는 것보다 좁았다.
**얼린 자리는 하나뿐이다**: 갈무리 익명 고지문(최박사 문장 대체표 §8 「변경하지 않는 자리」).
⚠ 그 표의 행은 글자 그대로 `session1~5` 공통이라 **`session6` 의 같은 문장은 표 밖**이다 —
여섯 중 하나만 바꾸면 같은 문장이 갈리므로 **여섯 다 유지**했다. **최박사 확인 사항.**

#### 12.6-d **인쇄 전용은 인쇄 전용이어야 한다**(U-7 · ADR-191)

`.print-only` 는 **인라인 스타일에 지고 있었다.** `ReportPrintHeader` 의 래퍼가 `display:'flex'` 를
인라인으로 들어 클래스를 이겼고, 그래서 「인쇄 전용」이라 적힌 문서 머리가 **화면에도 서 있었다** —
ADR-188 이 화면에 세우려고 더한 `screen` 소품은 **그동안 아무 일도 하지 않았다.**
`display: none !important` 로 고쳤다(인쇄 쪽도 `!important` 라 나중 규칙이 이긴다 — 순서가 지킨다).

그 뒤 **문서 화면의 이름 자리**는 라우트마다 다르다. 기준은 하나다 — **잃는 것이 없어야 한다.**

| 라우트 | 문서 머리 | 화면 이름 | 왜 |
|---|---|---|---|
| `/coach/…/report/[responseId]` | **화면에 선다**(`screen`) | 머리가 든다(`docTitle`) | **대상자 이름을 드는 곳이 머리뿐이다**(실측) |
...
`/report` 의 머리는 **회기 이름만 접는다**(`cohortOnScreen={false}`) — 띠의 칩이 이미 말한다.
**인쇄에는 남는다**: 종이 위에 어느 회기의 언제 자료인지가 없으면 문서가 아니다(ADR-69 계열).

#### 12.6-e **마무리 체크 독려**(U-8 · ADR-192)

독려의 축이 셋이다. **셋이 같은 문장을 쓴다.**

| 축 | 어디 | 무엇 |
|---|---|---|
| **참여자 화면** | `/home` · `/home/assessments` · `/my/cohorts` · 회기 대시보드 | 개시하면 **저절로** 뜬다(ADR-55). 문 하나 — `postJoinHref` |
| **인도자가 보내는 안내** | 회기 대시보드 「안내 보내기」 | 참여자 홈의 **확정 두 줄 + 주소**. Web Share → 클립보드 폴백 |
| **미완료 명단** | 회기 대시보드 「마무리 체크」 구획 | `n/총 완료` + 아직 안 한 이름. 미완료 0이면 **그리지 않는다** |

**왜 명단이 필요했나**: 3숫자(응답 완료·대기·돌봄)는 **wave 를 안 가른다** — 사전만 낸 사람도
「응답 완료」다. 마무리가 열려도 **누가 안 했는지 볼 길이 없었고**, 독려는 그 명단 없이 시작하지 못한다.

**문안을 새로 짓지 않았다.** 인도자가 보내는 안내는 참여자 홈이 쓰는 두 줄을 `_vocab/postNudge` 에서
그대로 읽는다 — **카톡에서 읽는 말과 로그인해서 보는 말이 같아야** 한다.
새 문안은 버튼 라벨 「안내 보내기」 하나뿐이고 **최박사 재가 대상**이다.

**낱말**(ADR-193 · 지휘부 답 2026-09-03) — 셋 다 이미 있던 확정 어휘를 읽는다.

| 자리 | 낱말 | 어디서 왔나 |
|---|---|---|
| 마무리 구획의 미완료자 | **미작성** | 갈무리 인도자 화면(`제출 / 작성 중 / 미작성`) |
| 참여자 대시보드의 마무리 | **시작** | 체크 허브가 마무리에 이미 쓰던 말 |
| 명단 묶음 | 「아직 안 함」 그대로 | 응답 0건 — **다른 뜻이라 다른 낱말로 갈랐다** |

⚠ **「작성 중」은 진단 응답에 없다.** `responses` 에 미제출 상태가 없고(제출 = 행 생성),
서버 초안은 인도자에게 **0행**이며(`response_drafts` 정책 넷이 전부 `user_id = auth.uid()`),
자동 저장은 `localStorage` 뿐이라 서버에 닿지 않는다 — RLS 를 열어도 뜨는 것은 *작성 중인 사람*이 아니라
***「중간 저장」 버튼을 눌러 본 사람***이다. 갈무리가 3단계인 것은 `checkin_save` 가 **자동으로 서버에 쓰기** 때문이다.
ADR-167 이 같은 3단계를 이미 기각했다(ADR-193 에 근거 전부).

#### 12.6-f **회기 관리는 접힘 블록이다**(U-10 · ADR-194)

「관리」는 **펼침·접힘**인데 화면에서 그렇게 읽히지 않았다 — 오른쪽 끝 작은 버튼이었고
열려도 라벨이 그대로에 `aria-expanded` 도 없어 **「두 화면」으로 읽혔다.**

**`core/ui/Disclosure` 로 갈아탔다** — 새로 그리지 않았다. 규격은 **ADR-88** 이 확정해 뒀고
그 부품 자체가 *같은 실패를 두 번 겪고* 나온 것이다: 줄 전체가 버튼 · 오른쪽에 **「펼치기」/「접기」 글자**
