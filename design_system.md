...
- 척도 레이블(minLabel·centerLabel·maxLabel)을 블록 **상단 고정(sticky)** 1회. 각 문항엔 원만.
- 각 문항은 **2단**: prompt(body) 한 줄(위) → 5도트(아래, **전체 폭 5등분**). 도트 지름 22px·**탭 타깃 ≥44px**(도트보다 큰 히트 영역).
- sticky 헤더와 각 행의 도트가 **동일 전체 폭**이라 minLabel↔1번 원·centerLabel↔3번 원·maxLabel↔5번 원 위로 정렬된다(좌우 분할 금지 — 열 어긋남 방지).
