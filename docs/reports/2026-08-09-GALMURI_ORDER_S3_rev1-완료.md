...
| 해시 | 내용 |
|---|---|
| **`42e4634`** | 구현 + `architecture.md` ADR-98 + 이 보고서 |
| `2c7ac21` | 대상 지시서(`docs/tasks/GALMURI_ORDER_S3_rev1.md`) |

> ADR-99 시정: 초판은 여기에 "해시는 채팅 보고에 포함"이라 적었다. **자기모순이었다** —
> 이 폴더의 존재 이유가 **채팅과 무관하게 감리 가능해야 한다**는 것이다(`CLAUDE.md` §11 여섯째).

## 8. 후속 (ADR-99)

이 보고서의 판단 하나가 뒤집혔다. **§5-③ `summaryFields` 마지막 자리 → 두 번째 자리.**

근거였던 "빈 칸이 목록 한복판에 생겨 읽는 흐름이 끊긴다"가 코드와 어긋났다 — 회차 현황은
`{s.cells.map((c, j) => (c.has ? … : null))}` 로 **빈 칸을 아예 렌더하지 않는다.** 그 상황은 발생하지 않는다.
자리는 카드 순서를 따른다(카드에서 `rough_project` 는 `gap_want` 바로 아래).

원 서술을 지우지 않고 여기 남기는 이유는 ADR-86·94 와 같다 — 지우면 무엇이 왜 뒤집혔는지가 사라진다.
