...
>
> **실측으로 고친 것 둘.**
> ⑴ `qa-coach` 는 가입 화면의 **`인도자로 신청할게요`** 체크로 신청서를 낸다(`AuthGate` 실측) —
>    기존 경로를 쓰므로 **DB 쓰기 0 원칙에 더 부합**한다.
> ⑵ **다만 그 체크만으로 끝나지 않는다**(지휘부 보완 · 실측 확인). **승인 큐가 둘이다** —
>    `/admin/approvals`(**회원 자격**)와 `/admin`(**인도자 신청**)이 별개이고,
>    `decide_coach_application` 이 승인 시 `users.role` 을 `'coach'` 로 **원자 승격**한다
>    (`20260628095527:45` — `role = 'user'` 인 경우에만. admin 불변).
>    **처음 절차서는 *"체크했으면 역할 부여를 건너뛰어도 된다"* 고 적었는데 틀렸다** —
>    체크는 신청서를 낼 뿐이고 승인이 따로 필요하다. 4단계를 두 갈래(4-가 체크한 경우 ·
>    4-나 잊은 경우)로 다시 썼다.
