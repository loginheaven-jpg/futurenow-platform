...
- **문안 회차 — 미룬 넷을 한 번에 올린다** (**언제: 서가 B 완주 직후**. 최박사 결재 대기).
  **★ 언제를 적어 둔다 — 「미룬 것이 목록에 없으면 잊힌다」**(지휘부 2026-08-29).
  서가 B 가 끝나면 **그 회차의 새 문안과 함께 한 번에** 올린다 — 문안 결재를 두 번 청하지 않는다.
  **한 자리에 모아 둔다** — 흩어 두면 다음 사람이 하나씩 발견하고 하나씩 지어낸다.
  넷 다 **9/6·9/20 어디에도 안 걸린다**(2026-08-29 실측으로 확인).

  | # | 자리 | 무엇 | 왜 미뤘나 |
  |---|---|---|---|
  | C2 | `AccountForm.tsx:329` | 「다시 로그인하지 **않으셔도 됩니다**」 — 표 §0 금지 어휘 | 표 밖 자리 · 뜻은 통한다 |
  | C3 | `session4.ts:128` · `session5.ts:134` | 대시가 **구조를 진다**(「인생의 원씽 — 세 원이…」 · 「… — 만약」) | 4·5회차는 **10/12·10/18** |
  | C4 | `programCopy.ts:20` · `galleryFixture.tsx:25` | 「1–2회차」 범위 기호 — 표 §0 이 허용한 것은 ` · `·` - ` 둘뿐 | **규칙에 없는 경우**다 |
  | **C5** | `cards.ts` `card-02`·`card-03` `alt` | `alt` 안의 **대시**(「매년 세웠다 **—** 3월이면」) | **C4 와 같은 계열**(지휘부 판정 2026-08-29) |

  **★ C5 를 지금 손대지 않는 이유가 규율이다** — `alt` 는 **표가 다루지 않은 자리**이므로
  지금 고치면 **결재 없는 문장을 짓게 된다.** C1 에서 표(R-01)를 찾아 그것으로 바꾼 것이
  바로 그 반대 행동이었다 — **「낡았으니 새로 쓰자」로 갔으면 문장을 하나 지어냈다.**
