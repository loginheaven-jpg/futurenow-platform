...
export function FeedClient({
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--space-5)', marginTop: 'var(--space-4)' }}>
      {/* 회기 전환 — **여럿일 때만 노출한다**(발주 §6.1). 하나뿐이면 고를 것이 없다.
          **실기기 2차 지적(2026-08-27): "클릭 자체가 안 된다."** 마크업은 정상 `<a href>` 였고
          ...
          선택 표시가 보이지 않았고, 양쪽 회기 모두 글이 0건이라 목록도 똑같았다.
      ...
      {posts.length === 0 ? (
        ...
        //   첫 글은 사람이 쓴다.
        //
        // **회기 이름을 문장에 넣는다**(실기기 2차 지적). 글이 0건이면 어느 회기를 보든 목록이
        //   똑같아서, 전환이 됐는지 화면이 말해 주지 않았다. 이름이 있으면 빈 화면조차 답을 한다.
        //   회기가 하나뿐이면 고를 것이 없으므로 이름을 붙이지 않는다(없는 선택을 암시하지 않는다).
