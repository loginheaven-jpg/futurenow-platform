// `/feed` 서버 컴포넌트 회귀 — **기수 전환이 실제로 화면을 바꾸는가**.
//
// 이 파일은 실기기 검증(2026-08-27)에서 나온 FAIL 1건의 회귀 잠금이다.
//   증상: 기수 칩을 눌러도 목록이 바뀌지 않는다.
//   원인: 칩은 같은 라우트에서 **쿼리만** 바꾸므로 FeedClient 가 재마운트되지 않고,
//        `useState(initialPosts)` 가 첫 값만 쓰므로 posts 가 옛 기수 글을 붙든다.
//        그런데 selectedCohortId 는 prop 이라 갱신된다 — **보는 기수와 쓰는 기수가 어긋난다.**
...
describe('/feed — 기수 전환이 화면을 바꾼다', () => {
  it('**FeedClient 가 기수로 keyed 돼 있다** — 이것이 없으면 칩을 눌러도 목록이 그대로다', async () => {
  ...
  it('**기수가 다르면 key 도 다르다** — 같으면 React 가 상태를 재사용해 결함이 되살아난다', async () => {
    ...
    expect(a?.key, '두 기수의 key 가 갈린다').not.toBe(b?.key);
  });

  it('선택된 기수의 글을 서버가 내려보낸다 — prop 과 key 가 같은 기수를 가리킨다', async () => {
    const b = findFeedClient(await render(COHORT_B));
    const props = b?.props as { selectedCohortId: string; initialPosts: { id: string }[] };
    expect(props.selectedCohortId, 'prop 도 전환된 기수다').toBe(COHORT_B);
    expect(props.initialPosts[0].id, '글도 그 기수 것이다').toBe(`post-of-${COHORT_B}`);
    // 이 셋이 어긋나면 '보는 기수'와 '쓰는 기수'가 갈린다 — 실기기에서 나온 그 증상이다.
    expect(b?.key).toBe(props.selectedCohortId);
  });

  it('모르는 기수 쿼리는 조용히 기본 기수로 떨어진다 — 추측해서 열지 않는다', async () => {
