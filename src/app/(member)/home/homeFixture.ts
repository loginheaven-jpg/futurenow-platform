...
/**
 * **시트 픽스처 — 화면 소품에서 껍데기 자료로 옮겼다**(U-2 §3).
 * 지우지 않고 옮긴다 — 지우면 *시트가 제대로 그려지는가* 를 아무도 재지 않게 된다.
 */
export const SHEET_FIXTURE: { name: string; role: string; cohort: string; groups: MenuGroup[]; chips: SessionChip[] } = {
  name: '김서온',
  role: '참여자',
  cohort: '예봄 2기',
  groups: [
