...
// ★★ **2026-09-02 판정으로 이 잠금의 무게가 옮겨 갔다.**
//
//   갈무리 화면의 열람 고지를 **여섯 회차 전부에서 걷었다** — 쓰는 순간에 「누가 본다」가
//   보이면 자기검열이 생긴다는 판단이다. **정책은 한 글자도 안 바뀌었다**(인도자·운영자는
//   전과 똑같이 읽는다). 바뀐 것은 **그 사실을 언제 말하는가**뿐이다.
//
//   그래서 **모집 자료가 유일한 고지처가 됐다.** 전에는 두 곳을 서로 묶어 놓았으나
//   이제 여기가 빠지면 **아무 데서도 말하지 않는다.** 잠금이 더 무거워졌지 가벼워지지 않았다.
describe('열람 범위 고지 — 모집 자료가 유일한 고지처다 (ADR-77 · 2026-09-02 개정)', () => {
  it('★★ 모집 고지가 인도자·운영자를 **둘 다** 명시한다 — 여기가 마지막 고지처다', () => {
    expect(ONLINE.foot, '모집 고지에 인도자가 없다').toContain('인도자');
    expect(ONLINE.foot, '모집 고지에 운영자가 없다 — 앱에서 걷었으므로 여기가 빠지면 아무 데서도 말하지 않는다').toContain('운영자');
  });

  it('★ 앱 갈무리에는 열람 고지가 **없다** — 되살아나면 두 곳이 되므로 다시 묶어야 한다', () => {
    const widen = (x: unknown) => x as { save: { notice2?: string } };
    expect(widen(CHECKIN_SESSION_1).save.notice2, '앱 고지가 되살아났다 — 이 잠금을 다시 묶어라').toBeUndefined();
  });
