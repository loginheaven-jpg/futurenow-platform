...
//   UUID 경로는 카톡으로 뿌릴 수도 구두로 안내할 수도 없다. 코드로 회기를 풀어 UUID 경로로 넘긴다.
//   정적 세그먼트 `values` 가 형제 `[session]` 보다 우선하므로 충돌하지 않는다.
//
// **비멤버 처리가 두 갈래다 — 이 갈림이 이 파일의 핵심이다.**
//   ① 인도자 회기 코드: 비멤버는 **정지**시킨다. 링크가 카톡으로 옮겨 다니므로 자기등록 표면을 열지 않는다
//      (ADR-89 — "링크만 있으면 누구나 실명제 회기에 자기등록"). 늦게 합류하는 사람은 인도자가 등록한다.
//   ② general(체험) 코드: 자기등록이 **목적**이다(`/join` 의 '체험 시작하기'와 같은 자리). 그래서 가입시킨다.
//      결과의 열람 범위도 다르다 — 체험 회기는 coach_id 가 운영자라 `본인 + 운영자`만 본다(ADR-63 전제).
