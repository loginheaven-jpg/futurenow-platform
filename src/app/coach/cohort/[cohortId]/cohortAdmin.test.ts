...
describe('refineActionError (실패 정제 — 원본 비노출)', () => {
  it('NotFound·권한·내부 CoreError → 통합 친화 메시지', () => {
    expect(refineActionError('회기를 찾을 수 없습니다: abc-123')).toBe('회기를 찾을 수 없거나 권한이 없어요.');
    expect(refineActionError('회기 수정은 코치 또는 운영자만 가능합니다')).toBe('회기를 찾을 수 없거나 권한이 없어요.');
    expect(refineActionError('updateCohort 실패: duplicate key value …')).toBe('회기를 찾을 수 없거나 권한이 없어요.');
  });
  it('친화 폴백 카피는 그대로 노출', () => {
