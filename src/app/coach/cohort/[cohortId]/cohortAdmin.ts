// 회기 관리 순수 헬퍼(서버액션·UI 공용 검증/정제 — 단위테스트 대상). 'use server' 아님(순수 export 허용).

/** 회기 이름 유효성: trim 후 1~40자. 빈값·공백전용·초과 거부. */
...
export function refineActionError(error?: string): string {
  ...
  // NotFound("회기를 찾을 수 없습니다: …") · Forbidden("…가능합니다") · 내부 CoreError("updateCohort 실패: …") → 통합(원본 비노출)
  if (error.includes('찾을 수 없') || error.includes('가능합니다') || error.includes('updateCohort 실패')) {
    return '회기를 찾을 수 없거나 권한이 없어요.';
  }
