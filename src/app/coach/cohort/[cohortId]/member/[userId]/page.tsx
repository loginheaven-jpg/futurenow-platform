...
export default async function CoachMemberJourneyPage({ params }: { params: Promise<{ cohortId: string; userId: string }> }) {
  ...
  return (
    <div className="journey-print-root" style={{ maxWidth: 720, margin: '0 auto', padding: 'var(--space-6) var(--space-4)' }}>
      ...
      {/* ★ **이름은 본문 첫 줄이 든다**(U-6). 아래 문서 머리는 이제 **정말로 인쇄 전용**이다 —
          `.print-only` 가 인라인 스타일을 이기도록 고쳤다(globals.css). 전에는 그 머리가 화면에도 서서
          같은 이름이 둘이었다. **잃는 것이 없다**: 대상자 이름은 `MemberJourney` 가 본문에 그리고,
          회기 이름은 띠의 칩이, 날짜는 «오늘» 이라 데이터가 아니다. */}
      <ConsoleTitle />
      <ReportPrintHeader
        title="갈무리 기록"
