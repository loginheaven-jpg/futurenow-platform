...
export default async function JoinPage({ searchParams }: { searchParams: Promise<{ cohort?: string | string[]; code?: string | string[] }> }) {
  const sp = await searchParams;
  const cohort = typeof sp.cohort === 'string' ? sp.cohort : null;
  const code = typeof sp.code === 'string' ? sp.code : null;
  return <JoinClient initialCohortId={cohort} initialCode={code} />;
}
