...
export interface LibraryItem {
  ...
  createdAt: string;
  /**
   * **이 자리에 사진을 그려도 되는가**(최박사 판정 2026-08-29 — 「전체공개시 사진」).
   *
   * **주소가 아니다.** 참·거짓 한 칸이고, 화면은 이미 있는 프록시 주소를 스스로 조립한다.
   *   그래서 위 §4 의 «주소가 없다» 가 **그대로** 산다 — 경로조차 오지 않는다.
   *   (피드는 `photoPath` 를 주고 따로 서명하지만, 서가는 프록시가 있어 그것도 필요 없다.)
   *
   * 서버가 넷을 곱해 낸다: 볼 수 있고 · 파일이고 · 실제로 이미지이고(mimetype) · 상한 안.
   *   **`canView` 가 거짓이면 반드시 거짓이다** — 목록에는 못 보는 자료도 서기 때문이다.
