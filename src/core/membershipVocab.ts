 ...
 *
 * 지휘부 원문: *"회차란 1기 2기 등 기수를 내가 잘못 표현한 것이다. **회기 소속**을 말한다.
 *   앞으로 **회기**로 표현하자."*
 *
 ...
 *      실기수 참여자가 신청할 때 본 말이라, 한쪽만 바꾸면 **신청 화면과 앱이 달라진다**
 *      (ADR-171 ② 가 회차 이름에서 같은 이유로 멈춘 선례다).
 *   ⑵ **인도자 콘솔**(「기수 대시보드」·「이 기수」)은 지시 맥락이 참여자 홈이었다.
 *   ⑶ **기수명 데이터**(`예봄 2기`)는 문안이 아니라 값이다 — 여기서 못 바꾼다.
