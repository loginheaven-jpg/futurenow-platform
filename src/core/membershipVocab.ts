 ...
 * > 회기 마감 트리거도 `held` 를 건드리지 않는다(*트리거가 운영자 판단을 뒤집지 않는다*).
 * > 판정에서 **무엇보다 먼저**다(`held > cohort > 저장 > pending`) — 그러지 않으면
 * > 회기에 등록되는 것만으로 보류가 무력화된다.
 ...
 *
 * **2026-08-30 최박사 확정으로 한 번 더 조여졌다** — *가입하시면* 이 아니라
 *   **가입해 포럼회원으로 승인받으시면** 이다. 앞 판은 가입만으로 열리는 것처럼 읽혔고
 *   그것은 `UPGRADE_HOWTO`(*가입하고 정회원자격을 취득하시면*)와 조건이 어긋났다.
 *   **내가 보고한 겹침이 이 문장으로 닫혔다.**
 */
export const TIER_LEAD: Record<MemberTier, string> = {
  visitor: '세미나에 참여하시는 동안, 또는 촉진자포럼에 가입해 포럼회원으로 승인받으시면 진단 등 모든 도구를 이용하실 수 있습니다.',
...
/**
 * **한 기(期)를 부르는 말** — 「회기」다(지휘부 확정 2026-09-02).
 *
 * 지휘부 원문: *"회차란 1기 2기 등 회기를 내가 잘못 표현한 것이다. **회기 소속**을 말한다.
 *   앞으로 **회기**로 표현하자."*
 *
 * ★ **새로 지은 말이 아니다.** 최박사 원문이 이미 이 말을 쓴다 —
 *   *"참여자는 **회기** 기간동안 도구를 사용할 수 있다"*(아래 `PARTICIPANT_LEAD` 주석).
 *   그러니 이것은 **없던 말을 만드는 것이 아니라 쓰던 말로 맞추는 것**이다.
 *
 * ★ **한 곳에 둔다**(불변식 23). 낱말을 화면마다 박으면 다음에 바꿀 때 반드시 한쪽이 남는다.
 *
 * ★ **바꾸지 않은 자리가 있다**(범위를 적어 둔다 — 다음 사람이 결손으로 보지 않게):
 *   ⑴ **모집 랜딩**(`recruit/*`)과 **갈무리 문항**(`checkin/session*`)은 **결재 문안**이다.
 *      실회기 참여자가 신청할 때 본 말이라, 한쪽만 바꾸면 **신청 화면과 앱이 달라진다**
 *      (ADR-171 ② 가 회차 이름에서 같은 이유로 멈춘 선례다).
 *   ⑵ **인도자 콘솔**(「회기 대시보드」·「이 회기」)은 지시 맥락이 참여자 홈이었다.
 *   ⑶ **회기명 데이터**(`예봄 2기`)는 문안이 아니라 값이다 — 여기서 못 바꾼다.
 *   셋 다 회차 보고에 건수와 함께 올린다.
 */
export const COHORT_WORD = '회기';
...
/**
 * 회기명 축약 — **이름 끝의 `n기` 를 뽑는다**(최박사 확정 2026-08-29).
 *
 * `퓨처나우2026예봄2기` → `2기`. 화면에는 `2기 참여자` 로 뜬다.
 *
 * **규칙이 이름 형식에 의존하므로, 끝이 `n기` 가 아닌 회기는 전체 이름을 그대로 쓴다**
...
//        `/home`(`targets[0].who`) · 회기 홈(`'참여자'` 하드코딩) · 진단 홈(`'참여자'` 하드코딩)
//   그 밖에는 없다. 확인한 것들:
 ...
 * 실제로 겸직자가 있으므로(운영 실측: 회기를 이끌면서 참여도 하는 사람 2명) 답이 필요하다.
 ...
 * 회기에 매이지 않아 `cohortRoles` 에 들어갈 수 없다(`cohortId` 가 없다).
