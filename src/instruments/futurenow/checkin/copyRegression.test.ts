...
describe('3회차 개정 1차가 되돌아가지 않는다 (ADR-98)', () => {
  ...
  it('심화 두 칸 — 부정 서술을 긍정형으로, 군더더기 축약', () => {
    expect(lits.has('고쳐야 할 것을 찾는 칸이 아닙니다. 어긋난 곳이 보이면 그것만으로 충분합니다.')).toBe(false);
    expect(lits.has('보인 것 하나면 충분합니다. 오늘은 알아차리는 데까지가 몫이에요.')).toBe(true);
    expect(lits.has('좋은 말이든 아니든 상관없습니다. 그냥 자주 나오는 말이면 됩니다.')).toBe(false);
    expect(lits.has('좋은 말이든 아니든, 그냥 자주 나오는 말이면 됩니다.')).toBe(true);
  });
