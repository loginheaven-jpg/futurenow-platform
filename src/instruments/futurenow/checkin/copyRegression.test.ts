...
describe('§3 다섯 자리는 지워지지 않았다 (ADR-102)', () => {
  ...
  // ★★★ **얼린 문장이 여섯에서 갈리지 않게 잠근다**(U-6).
  //
  //   U-6 이 저장소의 「차수」를 전부 「회기」로 옮기면서 **이 문장 하나만 남겼다** —
  //   최박사 문장 대체표 §8 「변경하지 않는 자리」에 유지로 박혀 있기 때문이다.
  //   ⚠ 그런데 그 표의 행은 글자 그대로 **`session1~5` 공통**이고, 살아 있는 코드에는
  //   **session6 에도 같은 문장이 있다**. 즉 여섯 중 하나는 확정 목록 **밖**이다.
  //
  //   여섯 중 하나만 고치면 **같은 고지문이 회차마다 달라진다** — 참여자에게 한 약속이
  //   회차에 따라 다른 말이 되는 것이라 어느 쪽으로든 갈리는 것이 가장 나쁘다.
  //   그래서 **여섯을 한 덩어리로 묶어 둔다.** 최박사가 판단하면 여섯을 함께 옮긴다.
  it('★★ 익명 고지문이 여섯 회차에서 **한 글자도 다르지 않다** (최박사 §8 · session6 은 표 밖)', () => {
    const FILES = ['session1', 'session2', 'session3', 'session4', 'session5', 'session6'] as const;
    const found = FILES.map((f) => [...koreanLiterals(f)].filter((l) => l.includes('이름 없이 전달합니다')));
    // ⑦ **잴 것이 실재하는가** — 회차마다 정확히 하나여야 한다(0이면 아래 비교가 공허하다).
    found.forEach((hits, i) => expect(hits.length, `${FILES[i]} 의 익명 고지문이 하나가 아니다`).toBe(1));
    const uniq = new Set(found.map((h) => h[0]));
    expect([...uniq], '회차마다 고지문이 갈렸다 — 여섯을 함께 옮기지 않으면 안 된다').toHaveLength(1);
  });
