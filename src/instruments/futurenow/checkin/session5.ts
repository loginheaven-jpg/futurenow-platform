  ...
  deepen: {
    ...
    fields: [
      ...
      },
      {
        ...
        // **주의 — 이 placeholder 는 지금 화면에 뜨지 않는다**(아래 domino_then 도 같다).
        //   CheckinCardClient 의 심화 렌더가 TextArea 에 placeholder 를 넘기지 않는다 — 1~4회차 심화에
        //   placeholder 선언이 0건이라 그 자리가 필요한 적이 없었고, 5회차가 처음 드러냈다.
        //   지시서가 지정한 값이라 선언은 남기고, 렌더 한 줄을 고칠지는 **지휘부 판단으로 넘긴다**
        //   — §9 수용 기준이 렌더 경로 변경 0을 못 박았다. session5.test.ts 가 이 상태를 명시로 잠근다.
        placeholder: '월요일 아침 사무실에 앉으면',
      },
      {
        key: 'domino_then',
        label: '나는',
        placeholder: '그 자료를 30분 먼저 연다', // 위와 같다 — 지금은 화면에 뜨지 않는다
