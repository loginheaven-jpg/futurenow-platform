...
--
-- **적용 버전 = `20260826092730`** (파일명 `20260826160000_` 와 다르다).
--   `apply_migration` 이 적용 시각으로 자체 채번하므로 파일을 만드는 시점에는 그 값을 알 수 없다.
--   사후에 파일명을 맞추는 것은 규율이 아니라 뒤처리이므로 **바꾸지 않고 여기 적는다**
--   — 대조가 필요할 때 파일 안에서 답이 나오게 한다(지휘부 확정 2026-08-26).
--   `value_assessments`(파일 20260826120000 → 적용 20260826005825)에 이어 **두 번째 사례**다.
--   감수할 수 있는 조건은 **상대 순서 보존** 하나뿐이다: 두 사례 모두 파일명 순서와 적용 순서가
--   같아 `db reset` 시 재정렬 위험이 없다. 그 조건이 깨지는 날에는 규칙을 다시 세워야 한다.
--
-- **핵심 — 저장과 산출을 상태마다 나눈다.**
