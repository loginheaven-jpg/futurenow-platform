...
--
-- ✅ **적용됨 2026-08-30**(최박사 승인 · 장부 `20260830090000` = 파일명과 동일 · ADR-147).
--
--    ⚠ **이 줄은 2026-08-30 적용 뒤에도 「아직 적용되지 않았다」로 남아 있었다.**
--      그리고 `toolGate` 가 *적용 금지가 파일 머리에 적혀 있다* 며 **그 거짓을 지키고 있었다** —
--      잠금이 있었는데도 틀렸다. **잠근 대상이 「그 문장이 있는가」였고 「사실인가」가 아니었기 때문**이다.
--      적용 상태는 **따라가야 하는 값**인데 **얼어야 하는 값처럼 박아 두었다**(`CLAUDE.md` §11).
--      → 잠금을 *적용 뒤 기록이 있는가* 로 옮겼다. 세 파일 모두 같은 형식으로 맞췄다.
--
--    적용 전 절차는 그대로 지켰다 — 파일만 쓰고 멈춰 최박사 승인을 따로 받았다.
--    지난번 `feed_reactions` 때 그 멈춤이 가짜 함수 이름을 잡았다.
--
...
--
--      ⚠ **절반이 확정됐다**(최박사 2026-08-30 · 갱신). **여전히 의도가 아니라 보류다.**
--
--        **대상은 확정됐다** — 최박사 원문: *"이용 보류 는 누구에게도 걸수 있다."*
--          즉 운영자도 `expired` 가 될 수 있다. `decide_membership` 이 **자기 자신**만 막으므로
--          다른 운영자가 걸 수 있고, 그 길은 이미 열려 있다.
--
--        **콘솔에서 무엇을 잃는지는 미정이다** — 걸린 운영자가 감독 권한을 함께 잃어야 하는지,
--          아니면 참여자 자격만 잃고 감독은 남아야 하는지에 답이 없다.
--          **그래서 이번에도 고치지 않았다**(지휘부 지시 2026-08-30). 아래 네 곳의 첫 `OR` 은
--          **그대로 두었고 코드는 한 줄도 바뀌지 않았다.**
--
--      **다음 사람이 이 통과를 확정된 설계로 오해하지 않게 여기에 적어 둔다.**
--      답이 오면 고칠 자리는 아래 네 곳의 첫 `OR` 이다.
--
