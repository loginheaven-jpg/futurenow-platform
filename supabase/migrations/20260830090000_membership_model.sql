...
--
-- **이 파일은 적용 여부를 말하지 않는다**(지휘부 판정 2026-08-30). 정본은 원장이고
--   파일이 복사하면 반드시 낡는다. 조회로 얻는다:
--     select version from supabase_migrations.schema_migrations where version = '20260830090000';
--
--   ⚠ **한 번 낡았다.** 적용 뒤에도 머리에 *적용 전* 이라는 주장이 남아 있었고
--     `toolGate` 가 **그 거짓을 요구하고 있었다** — 잠금이 있었는데도 틀렸다.
--     **잠근 대상이 「그 문장이 있는가」였고 「사실인가」가 아니었다.**
--     그래서 이제 파일은 상태를 아예 말하지 않는다(무엇을 바꾸는지와 되돌리는 법만 적는다).
--     ADR-147 이 이 변경의 기록이다.
--
