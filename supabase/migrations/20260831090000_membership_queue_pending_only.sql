...
--
-- ✅ **적용됨 2026-08-30**(최박사 승인 · 장부 `20260831090000` = 파일명과 동일).
--    **배포 순서를 지켰다** — 코드가 먼저 나가고(`main` `f437fbf` · 실배포 확인) 그 뒤에 적용했다.
--    검증 전항 통과 — ⑴ 오버로드 `list_membership_queue()` 하나 · ⑵ `authenticated` EXECUTE 있음 / `anon` 없음
--                     ⑶ 대기 1 → 1 · 갈래는 `pending` 뿐 · ⑷ `valid_until` 3 → 3 · ⑸ 비운영자 42501
--    **적용 전 롤백 예행을 먼저 했다** — 되돌리는 문이 옛 정의와 권한까지 그대로 복원했다.
--    **이 편집은 주석뿐이고 DDL 은 한 글자도 바뀌지 않았다**(불변식 15).
--
-- ─────────────────────────────────────────────────────────────────────────────
