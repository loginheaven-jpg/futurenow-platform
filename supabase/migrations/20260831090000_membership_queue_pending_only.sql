...
--     select version from supabase_migrations.schema_migrations where version = '20260831090000';
--
-- **검증 결과는 남긴다** — 한 번 일어난 사실이다.
--    **배포 순서를 지켰다** — 코드가 먼저 나가고(`main` `f437fbf` 실배포 확인) 그 뒤였다.
--    ⑴ 오버로드 `list_membership_queue()` 하나 · ⑵ `authenticated` EXECUTE 있음 / `anon` 없음
--    ⑶ 대기 1 → 1 · 갈래는 `pending` 뿐 · ⑷ `valid_until` 3 → 3 · ⑸ 비운영자 42501
--    롤백 예행이 먼저였고 옛 정의와 권한까지 복원됐다.
--
-- ─────────────────────────────────────────────────────────────────────────────
