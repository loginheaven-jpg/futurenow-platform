...
describe('문안 회차 결재분 (2026-08-30)', () => {
  ...
  it('★ ③ 5회차 「만약」 칸 — **되돌렸다. 결재를 다시 청한다**', () => {
    // ★ **제 권고 ㉯가 틀렸다.** 「만약」은 앞 문장의 꼬리표가 아니라
    //   **바로 아래 칸의 문두**였다. 참여자가 실제로 보는 것은 이렇다:
    //
    ...
    //     [ 그 자료를 30분 먼저 연다 ]
    //
    //   이어 읽으면 **「만약 월요일 아침 사무실에 앉으면, 나는 그 자료를 30분 먼저 연다」** —
    //   **완결된 한 문장**이다. 「만약」을 앞으로 옮기면 그 문장에서 「만약」이 사라진다.
    //   `session5.test.ts` 의 「심화 ②는 인접 두 칸으로 짝을 이룬다」가 이것을 잡았다.
    //
    //   **결재장에 이 짝 구조를 적지 않은 것이 내 잘못이다** — placeholder 와 다음 칸을 안 봤다.
    //   그래서 **되돌리고 다시 청한다.** 이 잠금은 «아직 옛 문장이다» 를 기억한다.
    const s = read('src/instruments/futurenow/checkin/session5.ts');
    expect(s, '결재 없이 다시 고쳤다').toContain('걸어 본다면 — 만약');
  });
  ...
  it('★ 참여자가 읽는 갈무리 문자열에 대시가 없다 — 주석은 세지 않는다', () => {
    ...
    // ★ `session5` 는 뺀다 — 그 대시는 **짝 구조를 지고 있어 되돌렸고 결재 대기 중**이다(위 ③).
