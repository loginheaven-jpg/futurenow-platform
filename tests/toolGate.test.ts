...
//
// **새로운 종류가 아니다**(지휘부 판정) — 선례 셋이 있다:
//   · `src/app/_lib/designTokens.test.ts` — 별칭 **선언 여부**를 잰다(가장 가까운 선례)
//   · `tests/adrNumbers.test.ts` — ADR 번호의 **문서 실재**를 잰다
//   · `tests/designParts.test.ts` — 부품 **목록과 실물**을 잰다
//
// ─────────────────────────────────────────────────────────────────────────────
// **이 잠금이 막는 것과 못 막는 것 — 정직하게 적는다.**
//
//   **막는 것**: 검증 항목이 **통째로 지워지는 것.** 적용 직전에 파일만 보는 사람이
//     무엇을 재야 하는지 모른 채 적용하는 일을 막는다.
//
//   **못 막는 것 둘**:
//     ⑴ **항목은 있는데 내용이 틀린 경우.** 예컨대 `EXPLAIN` 이라는 낱말은 있는데
//        엉뚱한 테이블을 보라고 적혀 있으면 여기서는 통과한다. 문자열의 존재만 재기 때문이다.
//     ⑵ **항목을 적어 놓고 실제로 안 재는 경우.** 목록이 있다고 검증이 돌지는 않는다 —
//        도는 것은 사람이고, 그 사람이 건너뛰면 이 테스트는 여전히 초록이다.
//
...
describe('membership_model 마이그레이션 — 적용 전 점검', () => {
  ...
  it('**적용 뒤 검증 항목을 파일이 들고 있다** — 여덟(클코1 다섯 + 지휘부 여섯, 중복 정리)', () => {
    expect(sql).toContain('적용 뒤 검증 항목');
    for (const item of [
      'EXPLAIN', 'InitPlan', '진실표 재검증', '만료일 실동작',
      'expired` 차단 실동작', '기존 화면 회귀', '양방향 실재', '오염 0', '롤백 문',
    ]) {
      expect(sql, `검증 항목에 ${item} 가 없다`).toContain(item);
    }
  });

  it('**테스트 계정이 필요한 항목이 그렇게 표시돼 있다** — 실계정으로는 잴 수 없다', () => {
    expect(sql).toContain('[계정]');
    expect(sql).toContain('실기수와 실계정은 건드리지 않는다');
  });

  it('**적용 사실이 파일 머리에 적혀 있다** — 대조가 파일 안에서 끝난다', () => {
    // **이 잠금은 한 번 거짓을 지켰다.** 적용 뒤에도 *아직 적용되지 않았다* 를 요구하고 있었고,
    //   그래서 파일이 틀린 채로 초록이었다. 잠근 대상이 **「그 문장이 있는가」**였지
    //   **「사실인가」**가 아니었기 때문이다. 적용 상태는 **따라가야 하는 값**이다(`CLAUDE.md` §11).
    //   이제 셋 다 *적용됨 <날짜>* 를 요구한다 — 다음에 새 파일을 더할 때는
    //   **적용 전이면 이 잠금을 그 파일에 걸지 않는다**(걸면 다시 같은 거짓을 지킨다).
    expect(sql.slice(0, 600)).toContain('적용됨 2026-08-30');
  });
  ...
  it('**승인 큐 갈래 축소가 배포 순서와 검증 넷을 함께 들고 있다**', () => {
    ...
    // **적용됐다**(2026-08-30). 잠금을 *적용 전* 에서 *적용 뒤 기록* 으로 옮긴다 — 의도적 변경이다.
    expect(q, '적용 사실이 파일에 없다').toContain('적용됨 2026-08-30');
  ...
  it('**이 잠금이 못 막는 것을 파일이 스스로 적어 둔다** — 믿고 확인을 건너뛰지 않게', () => {
    const self = readFileSync('tests/toolGate.test.ts', 'utf8');
    expect(self).toContain('항목은 있는데 내용이 틀린 경우');
    expect(self).toContain('항목을 적어 놓고 실제로 안 재는 경우');
    expect(self).toContain('목록이 있는가');
  });
});
